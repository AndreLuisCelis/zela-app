...
export class Home {
  ...
  handleReportSuccess(data: { title: string; category: string; location: string, photos: string[] }) {
  ...
  handleSponsorSuccess(option: SponsorOption) {
    const reportId = this.activeReportId();
    if (reportId) {
      const ok = this.zelaService.patrocinar(reportId, option);
      if (ok) {
